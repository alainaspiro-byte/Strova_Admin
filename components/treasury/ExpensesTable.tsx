...
import { exportExpensesToCsv } from '@/lib/treasuryCsv'
...
import { CreateExpenseInput, PeriodFilter, TreasuryExpense } from '@/lib/treasuryTypes'
...
export function ExpensesTable({ period, periodLabel }: ExpensesTableProps) {
  const expensesQuery = useExpenses(period)
  const createMut = useCreateExpense(period)
  const updateMut = useUpdateExpense(period)
  const deleteMut = useDeleteExpense(period)

  const [openForm, setOpenForm] = useState(false)
  const [editing, setEditing] = useState<TreasuryExpense | null>(null)
  const [formError, setFormError] = useState<string | null>(null)

  const [pendingDelete, setPendingDelete] = useState<TreasuryExpense | null>(null)
  ...
  const openEditForm = (row: TreasuryExpense) => {
    setEditing(row)
    setFormError(null)
    setOpenForm(true)
  }
  ...
  const handleSubmit = async (input: CreateExpenseInput) => {
    ...
    try {
      ...
      setFormError(
        treasuryErrorMessage(
          e,
          editing ? 'No se pudo actualizar el egreso' : 'No se pudo registrar el egreso',
        ),
      )
    }
  }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setDeleteError(treasuryErrorMessage(e, 'No se pudo eliminar el egreso'))
    }
  }

  const handleExport = () => {
    if (items.length === 0) return
    exportExpensesToCsv(items, period)
  }
    ...
    <section className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/[0.06] dark:bg-[#111827] dark:shadow-none">
      <header className="flex flex-col gap-3 border-b border-slate-200 px-4 py-4 dark:border-white/[0.06] sm:flex-row sm:items-center sm:justify-between sm:px-6">
        <div className="min-w-0">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-white">Egresos</h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg bg-rose-600 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-rose-700 dark:bg-rose-500/90 dark:hover:bg-rose-500"
          >
            <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-white/[0.04]">
            {isLoading && items.length === 0 ? (
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center text-sm text-slate-500 dark:text-white/40 sm:px-6">
              ...
              items.map((row) => (
                <tr key={row.id} className="transition-colors hover:bg-slate-50 dark:hover:bg-white/[0.02]">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 font-medium text-rose-600 tabular-nums dark:text-rose-400">
                    {formatCup(row.amount)}
                  </td>
                  <td className="px-4 py-3 text-slate-700 dark:text-white/80">
                    <div className="max-w-[260px] truncate" title={row.description}>{row.description}</div>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3">
                    <span className="inline-flex rounded-full border border-slate-300 bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-600 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-white/70">
                      {row.category}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-slate-500 dark:text-white/50">
                    <div className="max-w-[260px] truncate text-xs" title={row.notes ?? ''}>
                      {row.notes || '—'}
                    </div>
                  </td>
      ...
      <ConfirmDeleteDialog
        ...
        description={
          pendingDelete
            ? `Vas a eliminar el egreso de ${formatCup(pendingDelete.amount)} del ${formatDate(pendingDelete.date)}.`
            : ''
        }
