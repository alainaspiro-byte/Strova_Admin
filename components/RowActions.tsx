...
/** Limpia el teléfono para wa.me: solo dígitos, sin +, espacios ni guiones. */
...
export function RowActions({ sub, onChange: _onChange, onRemoteUpdate }: Props) {
  ...
  const [planId, setPlanId] = useState<number>(Number(sub.planId) || 0)
  ...
  const loadPlansForChange = async () => {
    setModal('change')
    setPlanId(Number(sub.planId) || 0)
    try {
      const list = await apiClient.getPlansCatalog()
      setPlans(list)
      if (!sub.planId && list.length) setPlanId(Number(list[0].id) || 0)
    } catch {
      setPlans([])
    }
  }

  const wa = buildWaUrl(sub)
